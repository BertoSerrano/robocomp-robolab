...
bool GenericMonitor::configGetString(const std::string prefix, const std::string name, std::string &value,
                                     const std::string default_value, QStringList *list) {
    return GenericMonitor::configGetString(communicator, prefix, name, value, default_value, list);
}

bool
GenericMonitor::configGetString(Ice::CommunicatorPtr communicator, const std::string prefix, const std::string name,
                                std::string &value, const std::string default_value, QStringList *list) {
    ...
    if (value.length() == 0) {
        if (default_value.length() != 0) {
            ...
            QString error = QString("Error: can't get configuration string for variable without default value: ") +
                            QString::fromStdString(compound);
            qDebug() << error;
            throw error;
        }
    }
