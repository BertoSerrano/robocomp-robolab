...
void SpecificWorker::getData(imgType &rgbMatrix, depthType &distanceMatrix, RoboCompJointMotor::MotorStateMap &hState,
...
void SpecificWorker::getDepthInIR(depthType &distanceMatrix, RoboCompJointMotor::MotorStateMap &hState,
                                  RoboCompGenericBase::TBaseState &bState) {
    // Not implemented
...
void
SpecificWorker::getImage(ColorSeq &color, DepthSeq &depth, PointSeq &points, RoboCompJointMotor::MotorStateMap &hState,
                         RoboCompGenericBase::TBaseState &bState) {
...
void SpecificWorker::getDepth(DepthSeq &depth, RoboCompJointMotor::MotorStateMap &hState,
                              RoboCompGenericBase::TBaseState &bState) {
    depthBuff.copy(depth);
}

void SpecificWorker::getRGB(ColorSeq &color, RoboCompJointMotor::MotorStateMap &hState,
                            RoboCompGenericBase::TBaseState &bState_) {
    printf("getting RGB\n");
...
void SpecificWorker::getXYZ(PointSeq &points, RoboCompJointMotor::MotorStateMap &hState,
                            RoboCompGenericBase::TBaseState &bState) {
    pointsBuff.copy(points);
}

void SpecificWorker::getXYZByteStream(imgType &pointStream, MotorStateMap &hState, TBaseState &bState) {
    // Not implemented
    qDebug() << "getXYZByteStream not implemented yet";
}
...
// 	}
    ...
    for (int i = 0; i < depthModes.getSize(); ++i) {
        ...
        printf("%d x %d @ %d (%d)\n", pSupportedMode->getResolutionX(), pSupportedMode->getResolutionY(),
               pSupportedMode->getFps(), depthModes[i].getPixelFormat());
        if (pSupportedMode->getResolutionX() == 640 and pSupportedMode->getResolutionY() == 480 and
...
bool SpecificWorker::openStream(SensorType sensorType, VideoStream *stream) {
    ...
    return !stream->isValid() ? false : true;

}

void SpecificWorker::initializeStreams() {
    if (depthB) {
        if (!openStream(SENSOR_DEPTH, &depth)) {
            qFatal("OPEN DEPTH STREAM FAILED!");
        ...
        while (openni::OpenNI::waitForAnyStream(streams, 2, &imageType, 0) == openni::STATUS_OK) {
            ...
            switch (imageType) {
                ...
                default:
                    ...
                    qDebug() << "Error in wait\n";
            }
            if (average < 25) {
                qFatal("RGBD is not working, obtaining black images");
            }
        }
        qDebug() << "Out of checkInitialization, ";
    }
...
void SpecificWorker::readDepth() {
    ...
    if (openniRc != openni::STATUS_OK) {
        qDebug() << "Read depth failed!\n%s\n";
        printf("Read depth failed!\n%s\n", OpenNI::getExtendedError());
        return;
    }
    if (depthFrame.getVideoMode().getPixelFormat() != PIXEL_FORMAT_DEPTH_1_MM &&
        depthFrame.getVideoMode().getPixelFormat() != PIXEL_FORMAT_DEPTH_100_UM) {
        printf("Unexpected depth frame format\n");
        qDebug() << "Unexpected depth frame format\n";
        return;
    }
